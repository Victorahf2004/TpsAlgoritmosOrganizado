...
{
public: // atributos
    static const int INITIAL_CAPACITY = 8500;
...
public:
    ...
    // métodos
    vector4() : capacity(INITIAL_CAPACITY), top_(-1) {} // construtor padrão de vector incializando a capacidade inicial e o índice

    vector4(const iterator &start, const iterator &end) : capacity(INITIAL_CAPACITY), top_(-1) // construtor para inicializar um vector com iteradores
    {
        int size = end - start;
    ...
    iterator insert(iterator pos, const T &value)
    {
        ...
        iterator last = iterator(&elements[top_]);
        iterator it = last;
